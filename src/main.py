...
def run_algorithm_with_timeout(algorithm_func, distance_matrix, timeout, algorithm_name):
    start_time = time.time()
    result = None
    if timeout is None:
        result = algorithm_func(distance_matrix, None)
    else:
        while time.time() - start_time < timeout:
            result = algorithm_func(distance_matrix, timeout)
...
def main(file_path, options, timeout):
    ...
    if '-t' in options or '-all' in options:
        #print("timeout main -t = ", timeout)
        print("Dimensão: ", dimension)
        print("TSP Solver - Twice-Around-the-Tree")
        print("------------------------------------------------------------------------")

        tatt_route, tatt_cost, flag_erro, tatt_time = run_algorithm_with_timeout(twice_around_tree, distance_matrix, timeout, "Twice-Around-the-Tree")
        if flag_erro == 1:
    ...
    if '-c' in options or '-all' in options:
        #print("timeout main -c = ", timeout)
        print("TSP Solver - Christofides")
        print("------------------------------------------------------------------------")
        ch_route, ch_cost, flag_erro, ch_time = run_algorithm_with_timeout(christofides, distance_matrix, timeout, "Christofides")
    ...
    if '-b' in options or '-all' in options:
        #print("timeout main -b = ", timeout)
        print("TSP Solver - Branch-and-Bound")
        print("------------------------------------------------------------------------")
        bnb_route, bnb_cost, flag_erro, bnb_time = run_algorithm_with_timeout(bnb_tsp, distance_matrix, timeout, "Branch-and-Bound")
...
def erro():
    print("Uso: python3 ./src/main.py <caminho_do_arquivo_teste> <opções de algoritmos> [tempo]")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 5:
        print("Erro: Mais que 5 parâmetros passados\n")
        erro()
        sys.exit(1)
    ...
    main(file_path, options, timeout)
